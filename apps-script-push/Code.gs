...
function makeWebFcmRequest_(event, member, token, accessToken) {
  ...
  const title = isWait
    ...
    : '코트 입장 안내';
  const body = isWait
    ? member.name + '님, 대기 1순위입니다. ' +
      courtNo + '번 코트가 다음으로 나올 예정이니 준비해 주세요.'
    : member.name + '님, ' +
...
function removeInvalidTokens_(tokens) {
  ...
  try {
    saveWebPushTokens_(
      loadWebPushTokens_().filter(function(record) {
...
function cleanToken_(value) {
  ...
  if (!token || token.length < 40 || token.length > 4096) return '';
  return token;
}
...
function getFcmAccessToken_(forceRefresh) {
  ...
  const expiresIn = Math.max(
    60,
    Number(tokenResponse.expires_in || 3600) - 300
  );
  cache.put(
