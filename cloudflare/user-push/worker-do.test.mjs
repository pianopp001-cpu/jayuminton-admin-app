...
assert.ok(src.includes("if (!['wait1_ready', 'court_assignment', 'admin_message'].includes(type))"), 'assignment push event allowlist regressed');
assert.ok(src.includes("event.type === 'admin_message'"), 'admin_message push text branch missing');
...
assert.ok(src.includes("if (!members.length || members.length > maxMembers)"), 'assignment target cardinality guard missing');
assert.ok(src.includes("const maxMembers = type === 'admin_message' ? 200 : 4;"), 'admin_message broadcast cardinality cap missing');
