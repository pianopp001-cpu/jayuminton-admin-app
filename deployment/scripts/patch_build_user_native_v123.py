...
for old, new in (
    ("v1.2.2-overlay.apk", "v1.2.3-confirmed-overlay.apk"),
    ("user-native-push-v1.2.2.txt", "user-native-push-v1.2.3.txt"),
    ...
    ("version=1.2.2", "version=1.2.3"),
    ("version_code=122", "version_code=123"),
    ("jayuminton_wait1_native_v122", "jayuminton_wait1_native_v123"),
    ("jayuminton_court_native_v122", "jayuminton_court_native_v123"),
):
    s = s.replace(old, new)
...
if overlay_start < 0 or overlay_end < 0:
    raise SystemExit("v123 overlay show method not found")
...
if s.count(old) != 1:
    raise SystemExit("v123 overlay acknowledgement anchor missing")
s = s.replace(old, new, 1)

for marker in (
    'VERSION="1.2.3"', 'VERSION_CODE="123"',
    'AtomicBoolean(false)',
    'CountDownLatch(1)',
    'finished.await(2500',
    'windowManager.addView(panel, params)',
    ...
    if marker not in s:
        raise SystemExit("missing native v1.2.3 marker: " + marker)

path.write_text(s, encoding="utf-8")
print("Prepared v1.2.3 with verified overlay add before repeating vibration.")
