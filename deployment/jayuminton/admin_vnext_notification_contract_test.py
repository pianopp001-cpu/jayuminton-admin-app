...
for forbidden in [
    ...
    if forbidden in source:
        raise SystemExit('obsolete four-person-only rule remains: ' + forbidden)
...
for size in range(1, 5):
    ...
    old_wait_one = ['p' + str(i) for i in range(size)]
    next_wait_one = ['w' + str(i) for i in range(size)]
...
print('admin-vNext notification contract supports 1-4 finished, court-promoted, and wait1-promoted members')
