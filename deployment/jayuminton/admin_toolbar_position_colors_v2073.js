...
function ensureBottomActive(){/* jmBottomActiveButtonV1: the always-visible bottom row was missing
"코트배정대기로" -- select 1+ members, press this, they move to 코트배정대기.
That action already exists as #jmUnlimitedToolbar [data-a="active"] (calls
actionStatus('active') -> setMemberStatus RPC); this button just proxies to
it the same way the 수동배정 button proxies to the hidden swap button, so
admin_layout_wait4_compact_v2076.js's bottom() can pick it up by ID and
place it first in the row. */
var r=root();if(!r)return false;var existing=document.getElementById('jmBottomActiveButton'),refresh=findButton('새로고침');if(!refresh)return false;var parent=refresh.parentElement;if(!parent)return false;if(!existing){existing=document.createElement('button');existing.id='jmBottomActiveButton';existing.type='button';existing.textContent='코트배정대기로';existing.className=refresh.className||'';existing.addEventListener('click',function(e){e.preventDefault();e.stopPropagation();var activeBtn=document.querySelector('#jmUnlimitedToolbar [data-a="active"]');if(activeBtn)activeBtn.click();else toast('멤버를 먼저 선택하세요.',true);});parent.appendChild(existing);}return true;}
function ensureBottomTeam(){/* jmBottomTeamButtonV1: single on/off "팀설정" button for the
bottom row -- 1회성(임시)팀 지정. Select 1+ members and press: if all of
them are already in a temp-pair group, this undoes it (temp-clear); other-
wise it creates one (temp). Reuses #jmUnlimitedToolbar's existing
[data-a="temp"]/[data-a="temp-clear"] buttons (setTemporary()/
clearTemporary()) via the same click-proxy pattern as 수동배정/코트배정대기
로, so no new placement/RPC logic is needed here -- just the toggle
decision and a single always-visible button instead of two buttons buried
in the scrolling QUICK ASSIGN panel. */
var r=root();if(!r)return false;var existing=document.getElementById('jmBottomTeamButton'),refresh=findButton('새로고침');if(!refresh)return false;var parent=refresh.parentElement;if(!parent)return false;if(!existing){existing=document.createElement('button');existing.id='jmBottomTeamButton';existing.type='button';existing.textContent='팀설정';existing.className=refresh.className||'';existing.addEventListener('click',function(e){e.preventDefault();e.stopPropagation();var ids=selectedIds();if(!ids.length){toast('멤버를 먼저 선택하세요.',true);return;}var s=state();var pairs=(s&&Array.isArray(s.tempPairs))?s.tempPairs:[];var teamed=new Set();pairs.forEach(function(g){[g&&g.members,g&&g.pairA,g&&g.pairB].forEach(function(a){(Array.isArray(a)?a:[]).forEach(function(id){teamed.add(String(id));});});});var allTeamed=ids.every(function(id){return teamed.has(String(id));});var target=document.querySelector('#jmUnlimitedToolbar [data-a="'+(allTeamed?'temp-clear':'temp')+'"]');if(target)target.click();else toast('팀설정 기능을 찾을 수 없습니다.',true);});parent.appendChild(existing);}return true;}
...
function maintain(){ensureToolbar();fixWait4Second();syncSelectionVisual();ensureBottomMove();ensureBottomActive();ensureBottomTeam();cleanQuickLayout();paintSlots();}
