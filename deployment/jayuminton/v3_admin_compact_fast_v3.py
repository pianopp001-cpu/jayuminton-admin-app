...
marker = 'JAYUMINTON_ADMIN_COMPACT_FAST_V3'
addon = r'''

/* JAYUMINTON_ADMIN_COMPACT_FAST_V3
...
*/
(function installAdminCompactFastV3(){
  if (typeof IS_ADMIN !== 'undefined' && !IS_ADMIN) return;
  if (window.__JAYUMINTON_ADMIN_COMPACT_FAST_V3__) return;
  window.__JAYUMINTON_ADMIN_COMPACT_FAST_V3__ = true;
...
/* JAYUMINTON_ADMIN_COMPACT_FAST_V3 */
/* Keep saving feedback visible without covering or blocking the court cards. */
...
/* Header controls stay on one row and use only the width they need. */
#adminApp>header .wrap{flex-wrap:nowrap!important;gap:5px!important}
.header-undo-button,.header-refresh-button{
...
/* Do not let position-auto-assign consume the whole mobile bar. */
.mobile-quick-bar{gap:5px!important}
...
.mobile-undo-button,.mobile-refresh-button{
  flex:0 0 auto!important;
  width:auto!important;
  min-width:0!important;
  padding:0 7px!important;
...
/* Admin member card contract: game count is never hidden. */
#adminApp [data-member-id] .member-game-count{
...
#adminApp [data-member-id] .name{
  min-width:0!important;
  overflow:hidden!important;
  text-overflow:ellipsis!important;
  white-space:nowrap!important;
}
#adminApp [data-member-id] .name:has(.member-new-inline){
  overflow:visible!important;
  text-overflow:clip!important;
  white-space:normal!important;
}
#adminApp .member-new-inline{
...
for path,needles in {
    script:[marker,'member.isNew === true','member-game-count','requestAnimationFrame(decorateAll)'],
    style:[marker,'.mobile-assign-button','.member-game-count','.admin-saving-overlay.show']
}.items():
    text=path.read_text(encoding='utf-8')
    for needle in needles:
        if needle not in text: raise SystemExit(f'missing {needle!r} in {path.name}')
print('ADMIN_COMPACT_FAST_V3_OK')
