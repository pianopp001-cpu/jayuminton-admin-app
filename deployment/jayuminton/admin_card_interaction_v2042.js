(function installJayumintonAdminMultiActionV2053(){
  ...
  window.__JAYUMINTON_ADMIN_TEAM_VISUAL_STABLE_V2054__=true;
  ...
  function ensureStyle(){
    ...
    s.textContent=''
      ...
      +'.jm-multi-action{position:fixed;left:50%;bottom:18px;transform:translateX(-50%);z-index:2147483645;width:min(540px,calc(100vw - 20px));background:#fff;border:1px solid #e2e8f0;border-radius:20px;padding:14px;box-shadow:0 18px 55px rgba(15,23,42,.28);font-family:inherit}'
      +'.jm-multi-head{display:flex;align-items:center;justify-content:space-between;gap:10px;margin-bottom:10px}.jm-multi-title{font-size:15px;font-weight:900;color:#111827}.jm-multi-count{font-size:12px;font-weight:800;color:#15803d;background:#f0fdf4;border-radius:999px;padding:5px 9px}.jm-multi-help{font-size:12.5px;color:#64748b;line-height:1.45;margin-bottom:10px}.jm-multi-actions{display:grid;grid-template-columns:minmax(108px,1.25fr) minmax(82px,1fr) 64px;gap:7px}.jm-multi-action button{border:0;border-radius:12px;min-height:44px;font-size:13px;font-weight:850;font-family:inherit;white-space:nowrap!important;word-break:keep-all!important;overflow-wrap:normal!important;padding:0 9px}.jm-do-move{background:#dcfce7;color:#166534}.jm-do-team{background:#d4a017;color:#fff}.jm-do-cancel{background:#f1f5f9;color:#475569}.jm-do-team:disabled{opacity:.38}';
  ...
  function absorb(state){if(!Array.isArray(state&&state.tempPairs))return;yellow={};state.tempPairs.forEach(function(p){tempIds(p).forEach(function(id){yellow[id]=1;});});renderYellow();}
  ...
  async function executeSwap(src,dst){try{await rpc('swapMembers',[null,src.map(String),dst.map(String)]);reset();if(typeof renderState==='function')renderState();renderYellow();toast(src.length+'명 교환 완료',false);}catch(e){toast(String(e&&e.message||e||'교환 실패'),true);}}
  async function executeMove(){var src=selected.slice(),dst=targets.slice();if(src.length!==dst.length)return;try{if(targetKind==='member'){await rpc('swapMembers',[null,src,dst.map(function(t){return String(t.id);})]);}else{for(var i=0;i<src.length;i++){var t=dst[i];await rpc('moveOrSwapMember',[null,String(src[i]),String(t.type),String(t.type==='wait'?Number(t.key)-1:t.key),'']);}}reset();if(typeof renderState==='function')renderState();renderYellow();toast(src.length+'명 이동/교환 완료',false);}catch(e){toast(String(e&&e.message||e||'이동/교환 실패'),true);phase='source';targets=[];targetKind='';renderGreen();renderPanel();}}
  ...
  function wrapRenderState(){
    if(typeof window.renderState!=='function'||window.renderState.__jmYellowStable)return;
    var original=window.renderState;
    var wrapped=function(){var out=original.apply(this,arguments);renderGreen();renderYellow();return out;};
    wrapped.__jmYellowStable=true;
    window.renderState=wrapped;
  }
  function watch(){var a=app();if(!a){setTimeout(watch,120);return;}ensureStyle();wrapRenderState();if(!a.__jmV2053Observer){var busy=false;a.__jmV2053Observer=new MutationObserver(function(){if(busy)return;busy=true;(window.requestAnimationFrame||setTimeout)(function(){busy=false;wrapRenderState();renderGreen();renderYellow();},0);});a.__jmV2053Observer.observe(a,{childList:true,subtree:true});}refreshTeams();}
