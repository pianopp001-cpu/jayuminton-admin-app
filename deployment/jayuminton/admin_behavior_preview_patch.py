...
#adminApp .jm-new-fullname{white-space:normal!important;overflow:visible!important;text-overflow:clip!important;max-width:none!important;width:auto!important;min-width:max-content!important;font-weight:800!important}
</style>
<script id="jayuminton-admin-behavior-preview-v9">
(function(){
  ...
  function groupedTargetTap(type,index,targetId,event){var set=selected();if(!set||!set.size)return false;var ids=Array.from(set),locs=ids.map(locationOf);if(!locs.every(function(x){return x&&x.type===locs[0].type&&Number(x.index)===Number(locs[0].index);}))return false;if(locs[0].type===type&&Number(locs[0].index)===Number(index))return false;var target=targetGroup(type,index),targetIds=[];if(targetId){var pos=target.indexOf(targetId);if(pos<0)return false;targetIds=target.slice(pos,pos+ids.length);if(targetIds.length!==ids.length){alert('교환할 상대 인원이 부족합니다.');return true;}}if(event){event.preventDefault();event.stopPropagation();event.stopImmediatePropagation();}if(targetIds.length){if(ids.length!==targetIds.length){alert('같은 인원 수끼리 선택해서 교환하세요.');return true;}var source=locs[0];var method=source.type==='court'&&type==='court'?'adjustCourtMembers':source.type==='wait'&&type==='wait'?'adjustWaitGroupMembers':'';if(!method){alert('코트와 대기조 사이는 빈 칸으로 이동해 주세요.');return true;}var args=source.type==='court'?[ADMIN_PIN_VALUE,Number(source.index),Number(index),ids,targetIds]:[ADMIN_PIN_VALUE,Number(source.index),Number(index),ids,targetIds];set.clear();runAction(method,args);return true;}return false;}
  function interceptTargets(){var a=app();if(!a||a.__jmGroupCapture)return;a.__jmGroupCapture=true;a.addEventListener('click',function(ev){var card=ev.target&&ev.target.closest&&ev.target.closest('[data-member-id]');if(!card||ev.target.closest('button.small'))return;var id=memberId(card),loc=locationOf(id);if(!loc)return;if(groupedTargetTap(loc.type,loc.index,id,ev))return;},true);}
  function fullNewNames(){if(typeof STATE==='undefined'||!STATE||!Array.isArray(STATE.members))return;STATE.members.forEach(function(m){if(!m||!(m.isNew||m.newMember||m.is_new||m.new===true))return;document.querySelectorAll('#adminApp [data-member-id="'+String(m.id)+'"]').forEach(function(card){var name=card.querySelector('.member-name,.quick-member-name,.partial-name,.name');if(name){name.textContent=String(m.name||m.fullName||'');name.classList.add('jm-new-fullname');}card.style.setProperty('min-width','max-content');});});}
...
for required in ['id="adminBottomControls"','id="adminBottomRefresh"','id="adminBottomAutoAssign"','>실행취소</button>','>새로고침</button>','>자동배정</button>','jayuminton-admin-behavior-preview-v9','jm-multi-selected','installGroupedSelection','fullNewNames']:
