...
# Restore full four-person one-shot grouping: clicked two = A, remaining two = B.
...
# setTempPairs must apply the physical 2+2 order.
...
b=b.replace("var side=[group.pairA,TEMP_PAIR_COLORS[index%TEMP_PAIR_COLORS.length]];\n      Array.prototype.forEach.call(document.querySelectorAll(selector),function(card){var id=idOf(card);if(side[0].indexOf(id)>=0){card.classList.add('jm-temp-pair');if(card.style&&card.style.setProperty)card.style.setProperty('--jm-temp-pair-color',side[1]);}});","[[group.pairA,TEMP_PAIR_COLORS[(index*2)%TEMP_PAIR_COLORS.length]],[group.pairB,TEMP_PAIR_COLORS[(index*2+1)%TEMP_PAIR_COLORS.length]]].forEach(function(side){Array.prototype.forEach.call(document.querySelectorAll(selector),function(card){var id=idOf(card);if(side[0].indexOf(id)>=0){card.classList.add('jm-temp-pair');if(card.style&&card.style.setProperty)card.style.setProperty('--jm-temp-pair-color',side[1]);}});});")
...
# Permanent team labels never render as text; permanent team remains double line.
...
bp.write_text(b)
print('SHARED_TEMP_PAIRS_V3_OK pairA+pairB=true wait=left-right court=top-bottom labels=none fixed=double-line')
