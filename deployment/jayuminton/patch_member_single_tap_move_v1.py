...
(function(){
  ...
  var REPLIED_KEY='jayuminton_member_replied_messages_v1';
  function repliedIds(){try{var v=JSON.parse(localStorage.getItem(REPLIED_KEY)||'[]');return Array.isArray(v)?v.map(String):[];}catch(_){return [];}}
  function markReplied(id){try{var ids=repliedIds();if(ids.indexOf(String(id))<0){ids.push(String(id));localStorage.setItem(REPLIED_KEY,JSON.stringify(ids.slice(-200)));}}catch(_){}}
  function ensureUi(){var box=document.getElementById('jmDirectMessageAlert');if(!popupVisible(box))return;var card=box.querySelector('.jm-direct-message-card');if(!card)return;var item=activeMessage();if(!item||!item.id)return;var old=card.querySelector('.jm-member-reply-wrap');if(old&&String(old.dataset.messageId||'')!==String(item.id)){old.remove();old=null;}if(old)return;var wrap=document.createElement('div');wrap.className='jm-member-reply-wrap';wrap.dataset.messageId=String(item.id);wrap.style.cssText='margin-top:8px;display:flex;flex-direction:column;gap:6px';if(repliedIds().indexOf(String(item.id))>=0){wrap.innerHTML='<div style="text-align:center;color:#64748b;font-size:12px;padding:6px 0">이미 답장을 보냈습니다.</div>';card.appendChild(wrap);return;}wrap.innerHTML='<button type="button" class="jm-member-reply-open" style="width:100%;min-height:36px;border:1px solid #cbd5e1;border-radius:9px;background:#f8fafc;font-weight:800">답장 (1회만 가능)</button><div class="jm-member-reply-form" style="display:none;gap:6px;flex-direction:column"><textarea class="jm-member-reply-input" maxlength="80" rows="2" placeholder="관리자에게 답장 (80자 이내, 1회만 보낼 수 있어요)" style="width:100%;box-sizing:border-box;border:1px solid #cbd5e1;border-radius:9px;padding:8px;font-size:14px"></textarea><button type="button" class="jm-member-reply-send" style="width:100%;min-height:36px;border:0;border-radius:9px;background:#2563eb;color:#fff;font-weight:900">답장 보내기</button></div>';card.appendChild(wrap);wrap.querySelector('.jm-member-reply-open').addEventListener('click',function(){this.style.display='none';var form=wrap.querySelector('.jm-member-reply-form');form.style.display='flex';setTimeout(function(){wrap.querySelector('.jm-member-reply-input').focus();},30);});wrap.querySelector('.jm-member-reply-send').addEventListener('click',async function(){if(sending)return;var a=session(),current=activeMessage(),input=wrap.querySelector('.jm-member-reply-input');var replyText=String(input&&input.value||'').trim().slice(0,80);if(!a||!current||!current.id)return;if(!replyText){alert('답장 내용을 입력해 주세요.');return;}sending=true;this.disabled=true;this.textContent='전송 중...';try{await server('memberReplyToMessage',[String(a.token),String(a.member.id),String(current.id),replyText]);markReplied(current.id);if(typeof confirmJmDirectMessage==='function')confirmJmDirectMessage();if(typeof showMemberSettingMessage==='function')showMemberSettingMessage('관리자에게 답장을 보냈습니다.');else alert('관리자에게 답장을 보냈습니다.');}catch(e){var msg=String(e&&e.message||e||'답장 전송에 실패했습니다.');if(msg==='reply_already_sent'){markReplied(current.id);msg='이미 답장을 보냈습니다.';}alert(msg);this.disabled=false;this.textContent='답장 보내기';}finally{sending=false;}});}
