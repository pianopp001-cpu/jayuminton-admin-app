...
patches=[
 ...
 HERE/'admin_vnext_member_fields_patch.py', HERE/'admin_vnext_ui_patch.py', HERE/'admin_vnext_script_patch.py',
 HERE/'admin_vnext_multiselect_patch.py', HERE/'admin_vnext_partial_court_ui_patch.py', HERE/'admin_vnext_recent_played_ui_patch.py',
]
...
required_code=["const SHEET_PAIR_HISTORY = 'PairHistory';","const ADMIN_VNEXT_EVENTS = Object.freeze({","function buildAdminVnextEvent_(type, memberIds, courtNo)","function publishAdminVnextEvents_(events)","function readAdminVnextEvents_()","WAIT_ONE_PROMOTED: 'WAIT_ONE_PROMOTED'","adminVnextEvents: readAdminVnextEvents_()","'IS_NEW', 'PUBLIC_MEMO', 'IS_SPONSOR', 'BUNDLE_ID'",'function adjustMemberGames(pin,id,delta)','function setBundle(pin,ids)','function clearBundle(pin,ids)','function addMemberUnlocked_(pin, name, gender, grade, experience, extra)','function updateMemberProfile(pin, memberId, name, gender, grade, experience, extra)',"publicMemo: String(member.publicMemo || '').slice(0, 40)","function readLastPlayedAtMap_()","member.lastPlayedAt = lastPlayedAtMap[member.id]","function readPartnerSummaryMap_(members)","member.partnerSummary = partnerSummaryMap[member.id]","for(let i=0;i<finalCourtIds.length;i++) for(let j=i+1;j<finalCourtIds.length;j++)","score+=1000000","if(!activeSet[pool[i]])continue"]
required_admin=['id="newIsNew"','id="newIsSponsor"','id="newPublicMemo"','게임횟수 +1','묶음 지정','>자동배정</button>','admin-vnext-bottom-bar','mobile-refresh-button']
required_script=['function increaseSelectedGames()','function setSelectedBundle()','function clearSelectedBundle()',"server('adjustMemberGames'","runAction('setBundle'","runAction('clearBundle'",'function handleCourtMemberTap(courtNo, memberId, event)','function handleWaitMemberTap(groupIndex, memberId, event)','toggleSelected(memberId);','function adminVnextMemberBadges(member)','function adminVnextCardName(member)',"state.courts[no].length > 0",'member.partnerSummary','function adminVnextRecentPlayed(member)']
...
print('[admin-vNext] all admin-only patches including notification contract, recent-play statistic and UI verified successfully')
