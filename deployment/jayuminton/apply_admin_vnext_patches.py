...
required_code=["const SHEET_PAIR_HISTORY = 'PairHistory';","const ADMIN_VNEXT_EVENTS = Object.freeze({","function buildAdminVnextEvent_(type, memberIds, courtNo)","'IS_NEW', 'PUBLIC_MEMO', 'IS_SPONSOR', 'BUNDLE_ID'",'function adjustMemberGames(pin,id,delta)','function setBundle(pin,ids)','function clearBundle(pin,ids)','function addMemberUnlocked_(pin, name, gender, grade, experience, extra)','function updateMemberProfile(pin, memberId, name, gender, grade, experience, extra)',"publicMemo: String(member.publicMemo || '').slice(0, 40)","function readLastPlayedAtMap_()","lastPlayedAt:","for(let i=0;i<finalCourtIds.length;i++) for(let j=i+1;j<finalCourtIds.length;j++)","score+=1000000","if(!activeSet[pool[i]])continue"]
